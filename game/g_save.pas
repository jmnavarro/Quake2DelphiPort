...
procedure WriteField1(f: integer; field: field_p; base: PByte);
var
  p: Pointer;
  len, index: Integer;
begin
...
procedure WriteField2(f: integer; field: field_p; base: PByte);
var
  len: Integer;
  p: Pointer;
begin
  ...
  case field^._type of
    F_LSTRING:
      ...
      begin
        len := strlen(PPChar(p)^) + 1;
        { NOTE: 2003-05-14 (SP):  Alternative as unsure the dereference of the
                                  already dereferecend pointer is an
                                  accurate conversion.
        FileWrite(F, PPChar(p)^, len); }
        FileWrite(F, PPChar(p)^^, len);
      end;
  end;
end;

procedure ReadField(f: integer; field: field_p; base: PByte);
var
  p: Pointer;
  len, index: Integer;
begin
...
==============
*)
procedure WriteClient(f: integer; client: gclient_p);
var
  field: field_p;
  temp: gclient_t;
begin
