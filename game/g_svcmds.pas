...
{ 1) 16-May-2003 - Scott Price (scott.price@totalise.co.uk)                  }
{    Minor changes to put in dependancy on CPas to try to remain as close to }
{    the consistency of as-close-to-original conversion as possible, with    }
{    some minor formatting changes to aid comparisons.                       }
{----------------------------------------------------------------------------}
...
begin
  gi.cprintf(nil, PRINT_HIGH, 'Svcmd_Test_f()'#10);
end;
...
begin
  ...
  begin
    if (s^ < '0') or (s^ > '9') then
    begin
      gi.cprintf(nil, PRINT_HIGH, 'Bad filter address: %s'#10, s);
      Result:= False;
      Exit;
    end;
...
begin
  if (gi.argc < 3) then
  begin
    gi.cprintf(nil, PRINT_HIGH, 'Usage:  addip <ip-mask>'#10);
    Exit;
  end;
  ...
  begin
    ...
    begin
      gi.cprintf(nil, PRINT_HIGH, 'IP filter list is full'#10);
      Exit;
    end;
    Inc(numipfilters);
  end;
...
begin
  if (gi.argc < 3) then
  begin
    gi.cprintf(nil, PRINT_HIGH, 'Usage:  sv removeip <ip-mask>'#10);
    Exit;
  end;

  if not StringToFilter(gi.argv(2), f) then
    Exit;
  ...
  for i:= 0 to numipfilters - 1  do
    ...
    begin
      ...
      Dec(numipfilters);
      gi.cprintf(nil, PRINT_HIGH, 'Removed.'#10);
      Exit;
    end;
  gi.cprintf(nil, PRINT_HIGH, 'Didn''t find %s.'#10, gi.argv(2));
end;
...
begin
  ...
  gi.cprintf(nil, PRINT_HIGH, 'Writing %s.'#10, name);
  ...
  if (IOResult <> 0) then
  begin
    gi.cprintf(nil, PRINT_HIGH, 'Couldn''t open %s'#10, name);
    Exit;
  end;

  Write(f, Format('set filterban %d'#10, [{Round}Trunc(filterban^.value)]));
...
begin
  cmd := gi.argv(1);
  if (Q_stricmp(cmd, 'test') = 0) then
    Svcmd_Test_f
  else if (Q_stricmp(cmd, 'addip') = 0) then
    SVCmd_AddIP_f
  else if (Q_stricmp(cmd, 'removeip') = 0) then
    SVCmd_RemoveIP_f
  else if (Q_stricmp(cmd, 'listip') = 0) then
    SVCmd_ListIP_f
  else if (Q_stricmp(cmd, 'writeip') = 0) then
    SVCmd_WriteIP_f
  else
